...
class CryptoOptimizer {
    ...
    setupDeepLinks() {
        window.openMobileWallet = (walletName, action = 'connect') => {
            const deepLinks = {
                metamask: `https://metamask.app.link/dapp/${window.location.href}?action=${action}`,
                trust: `trust://app_link?url=${encodeURIComponent(window.location.href)}&action=${action}`,
                coinbase: `https://go.cb-w.com/dapp?cb_url=${encodeURIComponent(window.location.href)}&action=${action}`
            };
